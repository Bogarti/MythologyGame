        ...
        private List<Vector2[]> ProcessVertices(PolygonCollider2D collider, Vertices v, List<Vector2[]> list, ref PolygonParameters p, ref int pathIndex)
        {
            ...
            if (p.Decompose)
            {
                ...
                for (int j = 0; j < points.Count; j++)
                {
                    ...
                    {
                        float xValue = (2.0f * ((v2[i].x - offset.x) / p.Rect.width));
                        float yValue = (2.0f * ((v2[i].y - offset.y) / p.Rect.height));
                        v2[i] = new Vector2(xValue * p.XMultiplier * Scale, yValue * p.YMultiplier * Scale);
                    }
            ...
            else
            {
                ...
                {
                    float xValue = (2.0f * ((v[i].x - offset.x) / p.Rect.width));
                    float yValue = (2.0f * ((v[i].y - offset.y) / p.Rect.height));
                    v[i] = new Vector2(xValue * p.XMultiplier * Scale, yValue * p.YMultiplier * Scale);
                }
